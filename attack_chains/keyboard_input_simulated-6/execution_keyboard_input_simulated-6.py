...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: key_code[/]")
    console.print(f"  Description: Virtual key code to simulate")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for key_code [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: key_code")
    user_params["key_code"] = user_input

    user_params["meterpreter_sessionid"] = metasploit_sessionid

    # Meterpreter command execution
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: keyevent[/]")
    confirm_action()
    try:
        metasploit_executor.keyevent(user_params["key_code"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
