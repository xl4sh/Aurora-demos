...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: component[/]")
    console.print(f"  Description: Target component (keyboard/mouse)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for component [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: component")
    user_params["component"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: action[/]")
    console.print(f"  Description: Control action (enable/disable)")
    default_val = ""
    user_input = console.input(
    ...
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: uictl[/]")
    confirm_action()
    try:
        metasploit_executor.uictl(user_params["component"], user_params["action"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
