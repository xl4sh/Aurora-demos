...
async def main():
    ...
    console.print(f"[bold cyan]\n📌[Sliver Executor] Step 19 Parameter Input[/]")
    ...
    if not user_input and False:
        raise ValueError("Missing required parameter: IsIOC")
    user_params["IsIOC"] = user_input

    # Sliver command execution
    console.print(f"[bold cyan]\n[Sliver Executor] Executing: upload[/]")
    confirm_action()
    try:
        await sliver_executor.upload(user_params["RemotePath"], user_params["Data"], user_params["IsIOC"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
    ...
    console.print(f"[bold cyan]\n📌[PowerShell Executor] Step 22 Parameter Input[/]")
    ...
    console.print(f"[bold cyan]\n📌[PowerShell Executor] Step 25 Parameter Input[/]")
