...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: DirPath[/]")
    console.print(f"  Description: Path of the directory to create")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for DirPath [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: DirPath")
    user_params["DirPath"] = user_input

    user_params["meterpreter_sessionid"] = metasploit_sessionid

    # Meterpreter command execution
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: mkdir[/]")
    confirm_action()
    try:
        metasploit_executor.mkdir(user_params["DirPath"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: DirectoryPath[/]")
    console.print(f"  Description: Target directory path on remote system")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for DirectoryPath [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: DirectoryPath")
    user_params["DirectoryPath"] = user_input

    user_params["meterpreter_sessionid"] = metasploit_sessionid

    # Meterpreter command execution
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: cd[/]")
    confirm_action()
    try:
        metasploit_executor.cd(user_params["DirectoryPath"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
