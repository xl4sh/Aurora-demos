...
```bash
python aurora/executor/generateExecution.py --input #YmlFilePath --output #OutputPath
```
- `#YmlFilePath` : Input path (can be a directory containing attack_chain.yml files or a single YAML file);
...
python generateExecution.py -i publish-2.0 -o results/scripts
...
python generateExecution.py -i publish-2.0/active_desktop_info_known-1 -o results/scripts
...
python generateExecution.py -i publish-2.0/active_desktop_info_known-1/attack_chain.yml -o results/scripts
```
