...
async def main():
    ...
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for FilePath [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
    ...
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for Algorithm [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
    ...
    confirm_action()
    try:
        metasploit_executor.checksum(user_params["FilePath"], user_params["Algorithm"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
