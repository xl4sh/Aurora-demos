...
async def main():
    ...
    console.print(f"  Description: Command to execute (e.g., /bin/ls -l)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for Command [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
    ...
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: execute[/]")
    confirm_action()
    try:
        metasploit_executor.execute(user_params["Command"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
