...
async def main():
    ...
    if not user_input and False:
        raise ValueError("Missing required parameter: RemotePath")
    user_params["RemotePath"] = user_input
    ...
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for Recursive [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: Recursive")
    user_params["Recursive"] = user_input
    ...
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for Force [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: Force")
    user_params["Force"] = user_input

    # Sliver command execution
    console.print(f"[bold cyan]\n[Sliver Executor] Executing: rm[/]")
    confirm_action()
    try:
        await sliver_executor.rm(user_params["RemotePath"], user_params["Recursive"], user_params["Force"])
