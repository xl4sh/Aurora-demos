...
async def main():
    ...
    default_val = "False"
    user_input = console.input(
        f"[bold]➤ Enter value for recurse [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: recurse")
    user_params["recurse"] = user_input
    ...
    default_val = "/tmp"
    user_input = console.input(
        f"[bold]➤ Enter value for output_dir [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: output_dir")
    user_params["output_dir"] = user_input
    ...
    confirm_action()
    try:
        await sliver_executor.download(user_params["remote_path"], user_params["recurse"], user_params["output_dir"], user_params["SessionID"])
