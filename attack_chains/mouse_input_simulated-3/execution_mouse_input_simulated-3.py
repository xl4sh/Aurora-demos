...
async def main():
    ...
    console.print(f"  Description: Mouse action (move/click)")
    default_val = ""
    user_input = console.input(
    ...
    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: x[/]")
    console.print(f"  Description: X coordinate (required for move action)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for x [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: x")
    user_params["x"] = user_input

    console.print(f"[bold cyan]\n📌[Meterpreter Executor] Step 8 Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: y[/]")
    console.print(f"  Description: Y coordinate (required for move action)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for y [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: y")
    user_params["y"] = user_input

    user_params["meterpreter_sessionid"] = metasploit_sessionid

    # Meterpreter command execution
    console.print(f"[bold cyan]\n[Meterpreter Executor] Executing: mouse[/]")
    confirm_action()
    try:
        metasploit_executor.mouse(user_params["action"], user_params["x"], user_params["y"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
