...
async def main():
    ...
    confirm_action("Confirm execution of the next step?")
    await sliver_executor.get_system(#{hosting_process}, #{config}, sliver_sessionid)
    print_status_message("Command execution completed!", status="success")

    confirm_action("Confirm execution of the next step?")
    await sliver_executor.registry_write(#{hive}, #{reg_path}, #{key}, #{hostname}, #{string_value}, #{byte_value}, #{dword_value}, #{qword_value}, #{reg_type}, sliver_sessionid)
    print_status_message("Command execution completed!", status="success")
