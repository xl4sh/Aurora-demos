...
async def main():
    ...
    console.print(f"[bold yellow]  Parameter: VarName[/]")
    console.print(f"  Description: Environment variable name (optional)")
    default_val = ""
    user_input = console.input(
        f"[bold]➤ Enter value for VarName [default: {default_val}]: [/]"
    ) or default_val
    if not user_input and False:
        raise ValueError("Missing required parameter: VarName")
    user_params["VarName"] = user_input
    ...
    confirm_action()
    try:
        metasploit_executor.getenv(user_params["VarName"], user_params["meterpreter_sessionid"])
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
